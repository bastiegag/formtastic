...
class FT_Duplicate {
	...
	public function __construct() {
		add_action( 'admin_action_duplicate_form', array( $this, 'duplicate_form' ) );
	...
	public function link( $actions, $post ) {
		if ( current_user_can( 'manage_options' ) && get_post_type() == 'formtastic' ) {
			$actions['duplicate'] = sprintf( '<a href="%s" title="%s" rel="permalink">%s</a>',
				'admin.php?action=duplicate_form&amp;post=' . $post->ID,
				__( 'Duplicate this form', 'formtastic' ),
				__( 'Duplicate', 'formtastic' )
			);
		}
	...
	public function duplicate_form() {
		...
		if ( ! ( isset( $_GET['post'] ) || isset( $_POST['post'] )  || ( isset( $_REQUEST['action'] ) && 'duplicate_post' == $_REQUEST['action'] ) ) ) {
			wp_die( __( 'No form to duplicate', 'formtastic' ) );
		}
		...
		if ( isset( $post ) && $post != null ) {
			...
			wp_die( __( 'Error creating form. The original form could not be found.', 'formtastic' ) . ' : ' . $post_id );
